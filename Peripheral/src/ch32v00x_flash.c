 ...
 *        FLASH_ERROR_WRP, FLASH_COMPLETE or FLASH_TIMEOUT.
 */
FLASH_Status FLASH_UserOptionByteConfig(uint16_t OB_IWDG, uint16_t OB_STDBY, uint16_t OB_RST,
  ...
  if (status == FLASH_COMPLETE)
   {
    ...
    OB->USER = OB_IWDG|
      (uint16_t)(OB_STDBY|(uint16_t)(OB_RST|(uint16_t)(OB_PowerON_Start_Mode|(uint16_t)0xC2)));

//    OB->USER = 0x08f7; //Default value RD7=nRST
//    OB->USER = 0x20df; //Default value RD7=GPIO
...
void FlashOption(volatile uint16_t *hold)
 {
...
void FlashOptionData(uint8_t data0, uint8_t data1)
 {
  volatile uint16_t hold[OPTION_SIZE];     // array to hold current values while erasing
  ...
  if (hold32p[1] == ob32p[1]) return; //nothing to do
...
void FlashOptionUser(uint16_t user)
 {
  ...
  uint32_t *ob32p = (uint32_t *) OB_BASE;
  hold32p[0] = ob32p[0];            // Copy RDPR and USER
  hold32p[1] = ob32p[1];            // Copy in the two Data values
  hold32p[2] = ob32p[2];            // Copy WRPR0 and WEPR1
  hold[1] = user;
  if (hold32p[0] == ob32p[0]) return; //nothing to do
