  ...
  if (NVIC_Priority_Group == NVIC_PriorityGroup_1)
   {
	...
	(1 << 7) | (NVIC_InitStruct->NVIC_IRQChannelSubPriority << 6));
     }
    else
     if (NVIC_InitStruct->NVIC_IRQChannelPreemptionPriority == 0)
      {
       NVIC_SetPriority(NVIC_InitStruct->NVIC_IRQChannel,
	 (0 << 7) | (NVIC_InitStruct->NVIC_IRQChannelSubPriority << 6));
      }
   }
#endif
